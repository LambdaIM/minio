...
var lambdaServerCmd = cli.Command{
	Name:   "lambda-server",
	Usage:  "start lambda object storage server",
	Flags:  append(ServerFlags, GlobalFlags...),
	Action: lambdaServerMain,
...
func lambdaServerMain(ctx *cli.Context) {
	...
	newObject, err := newLambdaObjectLayer(globalEndpoints)
...
func newLambdaObjectLayer(endpoints EndpointList) (newObject ObjectLayer, err error) {
	// For FS only, directly use the disk.

	fmt.Println("Got endpoints: ", endpoints)
	return NewLambdaFSObjectLayer(endpoints)
}
